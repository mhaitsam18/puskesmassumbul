...
<div class="box box-warning direct-chat direct-chat-warning">
  <div class="box-header with-border">
    <h3 class="box-title mb-0">
      <div class="row mb-0">
        <?php echo $dokter['d_fullname'] ?>
        <?php if ($dokter['is_online'] == 1) : ?>
          <div class="text-success text-bold" style="font-size: 24pt;"><sup>•</sup></div>
        <?php endif; ?>
      </div>
    </h3>
    <p class="mt-0">
...
<script type="text/javascript">
  ...
  function save() {
    ...
    $.post('<?php echo base_url() ?>konsultasi/save', {
      d_link: "<?php echo $dokter['permalink'] ?>",
      dokter: "<?php echo $dokter['d_id'] ?>",
      image: "<?php echo $dokter['d_image'] ?>",
  ...
  function del($id) {
    swal({
      ...
      .then((willDelete) => {
        $.post('<?php echo base_url() ?>konsultasi/hapus', {
          id: $id
        }, function(result) {
          reload();
          scrollToBottom();
  ...
  setInterval(function() {
    $.post('<?php echo base_url() ?>konsultasi/reload', {
      d_link: "<?php echo $dokter['permalink'] ?>",
      dokter: "<?php echo $dokter['d_id'] ?>",
      image: "<?php echo $dokter['d_image'] ?>"
    }, function(result) {
